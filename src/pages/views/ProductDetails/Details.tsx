...
const Details = () => {
  ...
  const { loading, error, singleBeer } = useSingleBeer(beerId ?? "");
  const [quantity, setQuantity] = useState(1);

  console.log(beerId, loading, error, singleBeer);
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Stack justifyContent="flex-start">
            <Content title={singleBeer.name ?? "Cargando..."} helper={singleBeer.tagline ?? "..."}>
              <List dense>
                <ItemList title="Elaborado por primera vez" value={singleBeer.first_brewed ?? ""} />
                <ItemList
            ...
            <Box>
              ...
              <Grid container spacing={1}>
                {(singleBeer.food_pairing ?? []).map((item, index) => (
                  <Grid item>
                    <Chip label={item} variant="filled" color="secondary" key={index} />
                  </Grid>
                ))}
              </Grid>
            </Box>
